<?php
	...
	{
		$obj = new EasyDB(chaxun_xukezheng);
		$obj->IDVars="id";
		$obj->SetLimit(1);
		$ninfo = getrow("select title from ".TABLEPRE."chaxun_xukezheng where id=".$_GET['id']);
		if($obj->Delete())
		{
			$js=new Javascript();
			$js->Begin();
	...
	$sqlstr_tt="select * from ".TABLEPRE."chaxun_xukezheng where id<>'' ".$swhere;	
		
	if ($jydw<>"")
		$sqlstr_tt.=" and jydw = '$jydw'";
	if ($daibiaoren<>"")
		$sqlstr_tt.=" and daibiaoren = '$daibiaoren'";	
	if ($xukezheng<>"")
		$sqlstr_tt.=" and xukezheng = '$xukezheng'";		
...
//------------分页设置------------------
    ...
    $filename=$PHP_SELF."?jydw=$jydw&daibiaoren=$daibiaoren&xukezheng=$xukezheng";
...
<TABLE class=tableout cellSpacing=1 cellPadding=6 width="98%" align=center 
      border=0>
        ...
        <TR class=header align=left>
          <TD colspan="2">许可证管理</TD>
		  <TD width="91%" >          	
		   检索：生产/经营单位  
		        <input type="text" name="jydw" value="<?=$jydw?>">
           法定代表人：
           <input type="text" name="daibiaoren" value="<?=$daibiaoren?>">　　          许可证编号:
           <input type="text" name="xukezheng" value="<?=$xukezheng?>">
		   <input type="submit" name="Submit" value="检索">		   　
		   <a href="<?=$_SERVER["PHP_SELF"]?>"><FONT COLOR=RED>显示全部</FONT>	</A>	|    <a href="xukezheng_add.php"><FONT COLOR=RED>发布</FONT>	</A>	  </TD>
          </TR> </form>
        <TR>
...
<tr class="header">
  <td width="5%">编号</td>
  <td width="15%">生产/经营单位</td>
  <td width="10%">法定代表人</td>
  <td width="20%" align="center">许可证编号</td>
...
?>
	...
	<td><? echo $rec[jydw]?></td>
    <td><? echo $rec[daibiaoren]?></td>
	<td><? echo $rec[xukezheng];?></td>
    <td><? echo $rec[enddate];?></td>
    
    
    <td width="9%" align="center"><a href="xukezheng_edit.php?id=<?=$rec[id]?>">编辑</a></td>
	<td width="7%" align="center"><a href="xukezheng_manage.php?action=del&id=<?=$rec[id]?>">删除</a></td>
	</tr>
<?
	}
?>
