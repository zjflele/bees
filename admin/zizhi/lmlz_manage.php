<?php
	...
	{
		$obj = new EasyDB(chaxun_lmlz);
		$obj->IDVars="id";
		$obj->SetLimit(1);
		$ninfo = getrow("select title from ".TABLEPRE."chaxun_lmlz where id=".$_GET['id']);
		if($obj->Delete())
		{
			$js=new Javascript();
			$js->Begin();
	...
	$sqlstr_tt="select * from ".TABLEPRE."chaxun_lmlz where id<>'' ".$swhere;	
		
	if ($title<>"")
		$sqlstr_tt.=" and title = '$title'";
	if ($shuzhong<>"")
		$sqlstr_tt.=" and shuzhong = '$shuzhong'";	
	if ($bianhao<>"")
		$sqlstr_tt.=" and bianhao = '$bianhao'";		
...
//------------分页设置------------------
    ...
    $filename=$PHP_SELF."?title=$title&shuzhong=$shuzhong&shuzhong=$shuzhong&bianhao=$bianhao";
...
<TABLE class=tableout cellSpacing=1 cellPadding=6 width="98%" align=center 
      border=0>
        ...
        <TR class=header align=left>
          <TD colspan="2">林木良种管理</TD>
		  <TD width="91%" >          	
		   检索：树种  
		        <input type="text" name="shuzhong" value="<?=$shuzhong?>">
           名称：
           <input type="text" name="title" value="<?=$title?>">　　          良种编号:
           <input type="text" name="bianhao" value="<?=$bianhao?>">
		   <input type="submit" name="Submit" value="检索">		   　
		   <a href="<?=$_SERVER["PHP_SELF"]?>"><FONT COLOR=RED>显示全部</FONT>	</A>	|    <a href="lmlz_add.php"><FONT COLOR=RED>发布</FONT>	</A>	  </TD>
          </TR> </form>
        <TR>
...
<tr class="header">
  ...
  <td width="20%">名称</td>
  <td width="10%">树种</td>
  <td width="20%" align="center">良种编号</td>
  <td width="20%" align="center">良种类型</td>
...
?>
    ...
    <td><? echo $rec[shuzhong]?></td>
	<td><? echo $rec[bianhao];?></td>
    <td><? echo $rec[leixing];?></td>
    
    
    <td width="9%" align="center"><a href="lmlz_edit.php?id=<?=$rec[id]?>">编辑</a></td>
	<td width="7%" align="center"><a href="lmlz_manage.php?action=del&id=<?=$rec[id]?>">删除</a></td>
	</tr>
<?
	}
?>
