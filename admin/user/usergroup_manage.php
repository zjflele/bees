<?php
	...
	$sqlstr_tt="select * from ".TABLEPRE."usergroup where id<>'' ";		
	$sqlstr_tt.=" order by id";
	//echo $sqlstr_tt;
...
<script language="javascript">
 ...
 {
    alert("暂停成功");
	$('status'+bm_id).innerHTML = "<input type='button' value='暂停' onclick='allow("+bm_id+")'>";
 }
 
 function ChangeToRefuse()
 {
 alert("启用成功");
	$('status'+bm_id).innerHTML = "<input type='button' value='正常' onclick='refuse("+bm_id+")'>";
 }
</script>
</head>
...
<?
	...
	$nn+=1;
?>
	<tr bgcolor="#F8F9FC"><td><?=$rec[id]?></td><td><? echo $rec[groupname]?></td>
	  <td width="10%" align="center"><a href="usergroup_edit.php?id=<?=$rec[id]?>">编辑</a></td>
	<td width="11%" align="center"><a href="usergroup_manage.php?action=del&id=<?=$rec[id]?>">删除</a></td>
	<td align="center"><? echo "<div id=\"status$rec[id]\">";
//-----------------------------审核------------------------
echo ($rec[status]==1)?"<input type=\"button\" value=\"正常\" onclick=\"refuse($rec[id])\" />":"<input type=\"button\" value=\"暂停\" onclick=\"allow($rec[id])\" />";
echo "</div>";?></td>
	</tr>
<?
	}
